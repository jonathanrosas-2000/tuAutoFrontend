...
const Tienda = () => {
  ...
  const [selectedMileage, setSelectedMileage] = useState<[number, number]>([
    0, 125000,
  ]);
  ...
  useEffect(() => {
    if (
      ...
      typeof router.query.brand === "string"
    ) {
  ...
  useEffect(() => {
    ...
    if (!!searchTerm) {
      aux = aux.filter(
        (item: carType) =>
          ...
          item?.brand.toLocaleLowerCase().includes(searchTerm)
      );
    }
    ...
    if (selectedMileage[0] !== 0 || selectedMileage[1] !== 125000) {
      aux = aux.filter(
        (item) =>
          item.mileage >= selectedMileage[0] &&
          item.mileage <= selectedMileage[1]
      );
    }
  ...
  return (
    <PrincipalLayout>
      ...
      />
      <Chatbot
        message={`🌟 ¡Bienvenido! ¿Te interesa alguno de nuestros autos? 🚗`}
        whatsAppMessage={`¡Hola! Estoy interesado(a) en uno de sus vehiculos`}
      />
      {loading ? (
        <Loader />
      ) : (
        <>
          {productsFromDB.length > 0 ? (
            <>
              ...
              />
              <p className="legend">
                * Los precios de nuestros productos están sujetos a condiciones
                específicas, incluyendo el tipo de financiamiento y otras
                variables relevantes. Para obtener una cotización precisa y
                personalizada, le recomendamos que se ponga en contacto con uno
